...
const (
	...
	banUnauthorized = 24 * time.Hour
	banPayment      = 7 * 24 * time.Hour
	banRateFallback = 30 * time.Minute
...
func (s *banState) set(authID string, entry banEntry) {
	...
	if current, ok := s.bans[authID]; ok && current.ResetAt.After(entry.ResetAt) {
		return
	}
	s.bans[authID] = entry
}
...
func handleUsage(raw []byte) ([]byte, error) {
	...
	now := time.Now()
	entry, ok := classifyFailure(record.Failure.StatusCode, record.ResponseHeaders, now)
	if !ok {
		return okEnvelope(map[string]any{})
	}
	bans.set(record.AuthID, entry)
	slog.Warn("xai-autoban: excluded credential", "auth_id", record.AuthID, "status", entry.StatusCode, "reason", entry.Reason, "reset_at", entry.ResetAt.Format(time.RFC3339))
	return okEnvelope(map[string]any{})
}

func classifyFailure(status int, headers http.Header, now time.Time) (banEntry, bool) {
	entry := banEntry{StatusCode: status, BannedAt: now}
	switch status {
	case http.StatusUnauthorized:
		entry.Reason = "unauthorized"
		entry.ResetAt = now.Add(banUnauthorized)
	...
	case http.StatusTooManyRequests:
		...
		if entry.ResetAt.IsZero() {
			entry.ResetAt = now.Add(banRateFallback)
...
func dispatchManagement(req pluginapi.ManagementRequest) pluginapi.ManagementResponse {
	...
	case method == http.MethodPost && strings.HasSuffix(strings.TrimRight(req.Path, "/"), managementPrefix+"/unban-all"):
		return jsonResponse(http.StatusOK, map[string]any{"ok": true, "removed": bans.clearAll(), "status": currentStatus()})
...
func publicAction(req pluginapi.ManagementRequest) pluginapi.ManagementResponse {
	...
	case "unban-status":
		...
		removed = bans.clearStatus(status)
	case "unban-many":
		ids := strings.Split(req.Query.Get("auth_ids"), ",")
		removed = bans.clearMany(ids)
	case "unban-all":
		removed = bans.clearAll()
	default:
...
func importSnapshot(raw []byte) pluginapi.ManagementResponse {
	...
	for _, item := range snapshot.Bans {
		...
		bans.set(item.AuthID, banEntry{StatusCode: item.StatusCode, Reason: item.Reason, BannedAt: bannedAt, ResetAt: resetAt})
		imported++
	}
...
func currentStatus() statusInfo {
	...
	for id, entry := range snapshot {
		items = append(items, banInfo{AuthID: id, StatusCode: entry.StatusCode, Reason: entry.Reason, BannedAt: entry.BannedAt.Format(time.RFC3339), ResetAt: entry.ResetAt.Format(time.RFC3339), RemainingSeconds: int64(entry.ResetAt.Sub(now).Seconds())})
	}
	sort.Slice(items, func(i, j int) bool { return items[i].ResetAt < items[j].ResetAt })
	return statusInfo{Plugin: pluginName, Version: pluginVersion, Count: len(items), Bans: items}
}
...
<head>
  ...
  <style>
    ...
    *{box-sizing:border-box}body{margin:0;background:var(--bg);color:var(--text);font-family:Inter,ui-sans-serif,system-ui,-apple-system,"Segoe UI",sans-serif;font-size:14px;letter-spacing:0}
    ...
    main{max-width:1440px;margin:auto;padding:20px 24px 36px}.stats{display:grid;grid-template-columns:repeat(5,minmax(150px,1fr));gap:12px;margin-bottom:16px}.stat{background:var(--surface);border:1px solid var(--line);border-radius:7px;padding:16px;box-shadow:var(--shadow)}.stat-label{font-size:12px;color:var(--muted);font-weight:600}.stat-value{font-size:28px;line-height:1.1;font-weight:750;margin-top:7px}.stat-total{border-left:4px solid var(--blue)}.stat-401{border-left:4px solid #1570ef}.stat-402{border-left:4px solid #f79009}.stat-403{border-left:4px solid #d92d20}.stat-429{border-left:4px solid #7f56d9}
    .toolbar{background:var(--surface);border:1px solid var(--line);border-radius:7px;box-shadow:var(--shadow);margin-bottom:14px}.toolbar-row{display:flex;align-items:center;gap:10px;padding:12px;flex-wrap:wrap}.toolbar-row+.toolbar-row{border-top:1px solid var(--line)}input[type=search]{height:36px;min-width:260px;flex:1;border:1px solid #bfc8d2;border-radius:6px;padding:0 11px;background:#fff;color:var(--text);font-size:14px}.custom-code{height:36px;width:122px;border:1px solid #bfc8d2;border-radius:6px;padding:0 10px;background:#fff;color:var(--text);font-size:14px}.segments{display:flex;border:1px solid #bfc8d2;border-radius:6px;overflow:hidden}.segments button{border:0;border-right:1px solid #bfc8d2;border-radius:0;background:#fff;color:#344054}.segments button:last-child{border-right:0}.segments button.active{background:#e8eef6;color:#101828;font-weight:700}
    ...
    .table-shell{background:var(--surface);border:1px solid var(--line);border-radius:7px;box-shadow:var(--shadow);overflow:hidden}.table-head{padding:11px 14px;border-bottom:1px solid var(--line);display:flex;align-items:center;gap:12px}.table-head strong{font-size:14px}.table-head span{color:var(--muted);font-size:13px}.table-wrap{overflow:auto;max-height:64vh}table{border-collapse:collapse;width:100%;min-width:920px}th,td{padding:10px 12px;text-align:left;border-bottom:1px solid #edf0f3;vertical-align:middle}th{position:sticky;top:0;background:#f8fafb;color:#475467;font-size:12px;font-weight:700;z-index:1}tbody tr:hover{background:#f9fbfc}td code{font-family:"SFMono-Regular",Consolas,monospace;font-size:12px;color:#344054}.check{width:38px;text-align:center}.badge{display:inline-flex;align-items:center;justify-content:center;min-width:45px;height:24px;border-radius:12px;font-weight:750;font-size:12px}.b401{color:var(--blue);background:var(--blue-bg)}.b402{color:var(--amber);background:var(--amber-bg)}.b403{color:var(--red);background:var(--red-bg)}.b429{color:#6941c6;background:#f4f3ff}.reason{color:#475467}.time{white-space:nowrap}.remaining{font-variant-numeric:tabular-nums;font-weight:650}.row-action{height:30px;padding:0 9px;font-size:12px}.empty{padding:52px;text-align:center;color:var(--muted)}
...
<body>
  ...
  <main>
    ...
    <section class="toolbar">
      ...
      <div class="toolbar-row">
        ...
        <input id="customStatus" class="custom-code" type="number" min="100" max="599" inputmode="numeric" placeholder="自定义 Code">
        <button onclick="filterCustomStatus()">筛选 Code</button>
        <button class="quiet-danger" onclick="unbanCustomStatus()">清除 Code</button>
    ...
    <section class="table-shell">
      ...
      <div class="table-wrap">
        <table><thead><tr><th class="check"><input id="selectPage" type="checkbox" title="选择当前页"></th><th>Auth ID</th><th>状态</th><th>原因</th><th>隔离时间</th><th>自动解禁</th><th>剩余时间</th><th>操作</th></tr></thead><tbody id="rows"></tbody></table>
  ...
  <script>
    ...
    function customStatus(){const raw=$('customStatus').value.trim();const status=Number(raw);if(!Number.isInteger(status)||status<100||status>599){setMessage('请输入 100-599 的状态码',true);return null}return status}
    function filterCustomStatus(){const status=customStatus();if(status===null)return;state.filter=String(status);state.page=1;document.querySelectorAll('#filters button').forEach(x=>x.classList.remove('active'));render();setMessage('已筛选 '+status+'，共 '+filtered().length+' 条')}
    function unbanCustomStatus(){const status=customStatus();if(status===null)return;unbanStatus(status)}
